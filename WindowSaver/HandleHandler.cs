...
namespace WindowSaver
{
    class HandleHandler
    {
        ...
        [DllImport("ntdll.dll")]
        private static extern NtStatus NtQuerySystemInformation(SYSTEM_INFORMATION_CLASS SystemInformationClass, IntPtr SystemInformation,
        ...
        public static string[] getAllPIDs()
        {
            int pCount = 0;
            Process[] procs = Process.GetProcessesByName("excel");
            string[] xLProcs = new string[procs.Length];
            for (int i = 0; i < xLProcs.Length; i++)
            {
                xLProcs[i] = procs[i].ProcessName;
            }
            return xLProcs;
        }
        ...
        {
            int bufferSize = 0x10000; // initial buffer size of 65536 bytes
            int actualSize;           // will store size of actual data written to buffer

            //initial allocation
            IntPtr pSysInfoBuffer = Marshal.AllocHGlobal(bufferSize);

            //query for handles (initial buffer size will probably not be enough)
            NtStatus queryResult = NtQuerySystemInformation(SYSTEM_INFORMATION_CLASS.SystemHandleInformation, pSysInfoBuffer, bufferSize, out actualSize);

            //keep calling until buffer is large enough to fit all handles
            while (queryResult == NtStatus.InfoLengthMismatch)
            {
                //deallocate space since we couldn't fit all the handles
                Marshal.FreeHGlobal(pSysInfoBuffer);

                //double buffer size. Actual size is variable, so can't use that
                bufferSize = bufferSize * 2;

                //allocate memory with new buffer size
                pSysInfoBuffer = Marshal.AllocHGlobal(bufferSize);

                //quuery for handles
                queryResult = NtQuerySystemInformation(SYSTEM_INFORMATION_CLASS.SystemHandleInformation, pSysInfoBuffer, bufferSize, out actualSize);
            }

            if (queryResult == NtStatus.Success)
            {
                return pSysInfoBuffer; //will free pSysInfoBuffer later
            }
            else
            {
                //other NtStatus, this shouldn't happen
                Marshal.FreeHGlobal(pSysInfoBuffer);
                return IntPtr.Zero;
            }
        }

        public static List<SYSTEM_HANDLE_INFORMATION> getHandles(Process targetProess, IntPtr pSysHandles)
        {
            ...
            //number of total system handles
            ...
            for (int i = 0; i < nHandles; i++)
            {
                ...
                if (currentHandleInfo.OwnerPID == (uint)targetProess.Id)
                {
